  ...
  if(topic.includes('class') && message.toString().includes('order')) {
    ...
    if(obj.hasOwnProperty('order')) {
      ...
      if(obj.order === "ALLOW_STREAM") {
        ...
        console.log(obj);
       if(obj.remoteStream == CurrentUser) {
        console.log('MATCHED')
        $('#remoteVideo').html(`
          <iframe width="100%" height="200" src="https://stream.futurelines.live:5443/WebRTCAppEE/publish_embed.html?name=${obj.remoteStream}" frameborder="0" allowfullscreen></iframe>
        `);
       } else {
